...
--Abilities	
	...
	local _BarbedShot, _BarbedShot_RDY, _BarbedShot_CD													= ConRO:AbilityReady(Ability.BarbedShot, timeShift);
		local _BarbedShot_CHARGES, _BarbedShot_MaxCHARGES, _BarbedShot_CCD, _BarbedShot_MCCD 				= ConRO:SpellCharges(Ability.BarbedShot);
	...
	local _CallPet, _CallPet_RDY						 												= ConRO:AbilityReady(Ability.CallPetOne, timeShift);	
	local _CobraShot, _CobraShot_RDY					 												= ConRO:AbilityReady(Ability.CobraShot, timeShift);	
	local _CommandPet, _CommandPet_RDY					 												= ConRO:AbilityReady(Ability.CommandPet, timeShift);
	...
	local _FlayedShot, _FlayedShot_RDY	 																= ConRO:AbilityReady(Ability.FlayedShot, timeShift);
		local _FlayersMark_BUFF					 															= ConRO:Aura(Buff.FlayersMark, timeShift);	
	local _ResonatingArrow, _ResonatingArrow_RDY	 													= ConRO:AbilityReady(Ability.ResonatingArrow, timeShift);
	local _Soulshape, _Soulshape_RDY																	= ConRO:AbilityReady(Ability.Soulshape, timeShift);
	local _WildSpirits, _WildSpirits_RDY																= ConRO:AbilityReady(Ability.WildSpirits, timeShift);
...
--Rotations
	if not _in_combat then
		...
		if _BarbedShot_RDY and _BarbedShot_CHARGES >= 1 and tChosen[Ability.ScentofBlood] and _BestialWrath_RDY and not _BestialWrath_BUFF and (_AspectoftheWild_BUFF or _AspectoftheWild_CD > 15) and ConRO:FullMode(_BestialWrath) then
	...
	if _WildSpirits_RDY and ConRO:FullMode(_WildSpirits) then
		tinsert(ConRO.SuggestedSpells, _WildSpirits;
	end
	...
	if _ResonatingArrow_RDY and ConRO:FullMode(_ResonatingArrow) then
		tinsert(ConRO.SuggestedSpells, _ResonatingArrow;
	end
	
	if _FlayedShot_RDY then
		tinsert(ConRO.SuggestedSpells, _FlayedShot;
	end
...
--Abilities
	...
	local _MendPet, _MendPet_RDY																		= ConRO:AbilityReady(Ability.MendPet, timeShift);	
	local _FeedPet, _FeedPet_RDY																		= ConRO:AbilityReady(Ability.FeedPet, timeShift);
...
--Abilities	
	local _AimedShot, _AimedShot_RDY								 									= ConRO:AbilityReady(Ability.AimedShot, timeShift);
		local _AimedShot_CHARGES, _, _AimedShot_CCD, _AimedShot_MCCD										= ConRO:SpellCharges(Ability.AimedShot);
	...
	local _SniperShot, _SniperShot_RDY																	= ConRO:AbilityReady(PvPAbility.SniperShot, timeShift);
...
--Conditions
	...
	local _RapidFire_Threshold = 70
		if tChosen[Ability.Streamline] then
...
--Rotations
	...
	if _SteadyShot_RDY and currentSpell == _SteadyShot and ConRO.lastSpellId ~= _SteadyShot and tChosen[Ability.SteadyFocus] and (not _SteadyFocus_BUFF or _SteadyFocus_DUR <= 4 or _Trueshot_BUFF) then
		tinsert(ConRO.SuggestedSpells, _SteadyShot;
	end
	...
	if _SteadyShot_RDY and tChosen[Ability.SteadyFocus] and not _SteadyFocus_BUFF and currentSpell ~= _SteadyShot then
		tinsert(ConRO.SuggestedSpells, _SteadyShot;
	end
	...
	if ConRO_AoEButton:IsVisible() then
		if _TrickShots_BUFF then
			if _RapidFire_RDY and tChosen[Ability.Streamline] then
				tinsert(ConRO.SuggestedSpells, _RapidFire;
			end
...
--Abilities	
	local _Exhilaration, _Exhilaration_RDY					 											= ConRO:AbilityReady(Ability.Exhilaration, timeShift);
	local _AspectoftheTurtle, _AspectoftheTurtle_RDY		 											= ConRO:AbilityReady(Ability.AspectoftheTurtle, timeShift);
	local _SurvivaloftheFittestLW, _SurvivaloftheFittestLW_RDY											= ConRO:AbilityReady(Ability.SurvivaloftheFittestLW, timeShift);
...
--Rotations	
	...
	if _MendPet_RDY and _Pet_summoned and _Pet_Percent_Health <= 60 then
		return Ability.MendPet;
	end
	
	if _AspectoftheTurtle_RDY then
		return Ability.AspectoftheTurtle;
	end
	
	if _SurvivaloftheFittestLW_RDY and _LoneWolf_FORM and _in_combat then
		return Ability.SurvivaloftheFittestLW;
	end
...
--Abilities	
	...
	local _KillCommand, _KillCommand_RDY				 												= ConRO:AbilityReady(Ability.KillCommand, timeShift);
		local _KillCommand_CHARGES, _, _KillCommand_CCD														= ConRO:SpellCharges(Ability.KillCommand);	
	...
	local _WildfireBomb, _WildfireBomb_RDY																= ConRO:AbilityReady(Ability.WildfireBomb, timeShift);
		local _WildfireBomb_CHARGES, _, _WildfireBomb_CCD													= ConRO:SpellCharges(Ability.WildfireBomb);		
		local _WildfireBomb_DEBUFF																			= ConRO:TargetAura(Debuff.WildfireBomb, timeShift + 1);
	local _PheromoneBomb, _PheromoneBomb_RDY															= ConRO:AbilityReady(Ability.PheromoneBomb, timeShift);
		local _PheromoneBomb_DEBUFF																			= ConRO:TargetAura(Debuff.PheromoneBomb, timeShift + 1);
	local _ShrapnelBomb, _ShrapnelBomb_RDY																= ConRO:AbilityReady(Ability.ShrapnelBomb, timeShift);
	...
	local _TarTrap, _TarTrap_RDY																		= ConRO:AbilityReady(Ability.TarTrap, timeShift);
		local _TarTrap_DEBUFF																				= ConRO:TargetAura(Debuff.TarTrap, timeShift);
	local _VolatileBomb, _VolatileBomb_RDY 																= ConRO:AbilityReady(Ability.VolatileBomb, timeShift);
	...
	local _Chakrams, _Chakrams_RDY																		= ConRO:AbilityReady(Ability.Chakrams, timeShift);
...
--Rotations
	...
	if _Chakrams_RDY then
		tinsert(ConRO.SuggestedSpells, _Chakrams;
	end
	...
	if _Harpoon_RDY and tChosen[Ability.TermsofEngagement] then
	...
	if not tChosen[Ability.WildfireInfusion] then	
